...
kids = [
    ...
    Pos(0, 0, M2E["planeA"][0]) * cannon_pinion_b(),
    Pos(ml["minute"][0], ml["minute"][1], M2E["planeA"][0]) * minute_wheel_b(),
    Pos(0, 0, M2E["planeB"][0]) * hour_wheel_dial_b(),
    Pos(ml["s1"][0], ml["s1"][1], M2E["moon_p"][0]) * moon_s1_b(),
    Pos(ml["disc"][0], ml["disc"][1], M2E["moon_d"][0]) * moon_disc_b(),
    Pos(0, 0, M2E["platform"][1]) * dial_platform(),
    # barrel group (drum internals; arbor port pending 2e-half)
    Pos(bx, by, drum_z) * m1.drum(),
    Pos(bx, by, drum_z + 2.9) * m1.mainspring(),
    Pos(bx, by, drum_z + 19.4) * m1.cover(),
    # bridge side
    Pos(0, 0, 21 + zo) * broad_wave_bridge(),
    Pos(0, 0, 16.5 + zo) * pallet_cock(),
    Pos(0, 0, 29 + zo) * balance_cock_b(),
    Pos(bx, by, 23.5 + zo) * ratchet_b(),
    Pos(cwx, cwy, 23.5 + zo) * crown_wheel_b(),
    Pos(sp[0], sp[1], 29.5 + zo) * Rot(0, 0, 285) * stem_crown(),
    Pos(bx, by, 24.2 + zo) * Rot(0, 0, -30) * click_b(),
]
...
for key, r, z0, z1 in PH:
    x, y = m[key]
    kids.append(Pos(x, y, PLATE_T + z0) * Cylinder(r, z1 - z0, align=B)
                if key != "balance" else
                Pos(x, y, z0) * Cylinder(r, z1 - z0, align=B))
asm = Compound(label="k1_revb_full_r2", children=kids)
export_step(asm, "exports/revb/review_full_movement_r2.step")
